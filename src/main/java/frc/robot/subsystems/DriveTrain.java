...
public class DriveTrain extends SubsystemBase {
    ...
    // Pixy support variables
    private Boolean pixyChaseSteer = false;
    private double steeringAdjustment = 0.0;
    public int pixyChaseColor = Pixy2CCC.CCC_SIG1;
    private double pixyTargetSpeed = 0;
    ...
    */
    ...
    public void periodic() {
        ...
        if (pixyChaseSteer) {
            Block b = PixyCamera.getBiggestBlock(this.pixyChaseColor);
            ...
            if (b != null) {
                ...
                // the following speed control is imperfect and needs a great deal of tuning!
                this.pixyTargetSpeed = 1.0 - ((double) (b.getWidth() * b.getHeight()) / (double) (1400));
                if (pixyTargetSpeed > 0.8)
                    pixyTargetSpeed = 0.8;
                if (pixyTargetSpeed < 0.4)
                    pixyTargetSpeed = 0.4;

                SmartDashboard.putNumber("Pixy Target Speed", pixyTargetSpeed);
                SmartDashboard.putNumber("Pixy width", b.getWidth());
                SmartDashboard.putNumber("Pixy height ", b.getHeight());
            } else {
    ...
    public void driveWithJoystick(Joystick stick) {
        ...
        y = Deadband(y);
        // tankDrive.arcadeDrive(-Math.pow(y, 5),Math.pow(twist, 5));

        if (pixyChaseSteer) {
            // tankDrive.arcadeDrive(-y, steeringAdjustment/160.0 * 1.3);
            if (PixyCamera.blockCount > 0) {
                // The following line will approach balls using auto stearing and auto speed
                // control.
                tankDrive.arcadeDrive(this.pixyTargetSpeed, steeringAdjustment / 160.0 * 1.3);
                // The following line will approach balls using auto steering and user speed
                // control.
                // tankDrive.arcadeDrive(-y, steeringAdjustment/160.0 * 1.3);
            } else {
            ...
            tankDrive.arcadeDrive(-y, twist, true);
        }
    }
    ...
    public boolean closedLoopTurnComplete(double target) {
        ...
        // Print outs for tuning turn PID
        // System.out.println("test/shifter/Turn Distance Remaining" + Math.abs(ypr[0] -
        // target));
        // System.out.print("ypr=" + ypr[0]);
        // System.out.println(" target=" + target);
        return Math.abs(ypr[0] - target) < 1; // allow 1 degree range around target
    }
